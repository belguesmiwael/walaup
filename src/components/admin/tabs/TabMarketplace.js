...
const EMPTY_FORM = {
  ...
  price_from: '', demo_url: '', owner_type: 'walaup', active: true, icon: '📱', sort_order: 0,
}
...
const sEmptyMsg  = { padding: 40, textAlign: 'center', color: 'var(--tx-3)', fontSize: 13 }
const sActBtns   = { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }
...
const sIconInput = { fontSize: 20, textAlign: 'center' }
const sSaveBtn   = { width: '100%', padding: 12, borderRadius: 11, background: 'linear-gradient(135deg,#6366F1,#8B5CF6)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', marginTop: 6, fontFamily: "'Space Grotesk',sans-serif" }
const sSaveBtnDis = { ...sSaveBtn, opacity: 0.6, cursor: 'not-allowed' }
...
export default function TabMarketplace() {
  ...
  const fetchApps = async () => {
    setLoading(true)
    const { data } = await supabase.from('marketplace_apps').select('*').order('sort_order', { ascending: true })
    setApps(data || [])
    setLoading(false)
  }
  ...
  const openAdd  = () => { setForm({ ...EMPTY_FORM, sort_order: apps.length + 1 }); setEditApp(null); setShowModal(true); setMsg(null) }
  const openEdit = (app) => { setForm({ ...app, price_from: String(app.price_from || '') }); setEditApp(app); setShowModal(true); setMsg(null) }
  ...
  const handleSave = async () => {
    ...
    const payload = { ...form, price_from: parseFloat(form.price_from) || 0, sort_order: parseInt(form.sort_order) || 0 }
    let error
    if (editApp) {
  ...
  const CSS = `
    ...
    .adm-mp-hactions { display:flex; gap:8px; }
  ...
  return (
    <>
      ...
      <div className="adm-mp">
        ...
        {loading ? (
          <div style={sEmptyMsg}>Chargement…</div>
        ) : apps.length === 0 ? (
          <div style={sEmptyMsg}>Aucune app. Ajoutez votre première application.</div>
        ) : apps.map(app => {
          const sOwnerBadge = {
            padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 700,
            color:       app.owner_type === 'walaup' ? '#6366F1' : '#F59E0B',
            background:  app.owner_type === 'walaup' ? 'rgba(99,102,241,0.12)' : 'rgba(245,158,11,0.12)',
          }
          return (
            <div key={app.id} className={`adm-app-row${!app.active ? ' adm-inactive' : ''}`}>
              ...
              <div style={sActBtns}>
                ...
                <span style={sOwnerBadge}>{app.owner_type === 'walaup' ? '🔵 Walaup' : '🟡 Partenaire'}</span>
                {app.demo_url && (
          ...
          <div className="adm-mpc" onClick={e => e.stopPropagation()}>
            ...
            <div className="adm-form-2col">
              <div className="adm-form-row">
                ...
                <input className="adm-form-inp" value={form.icon} onChange={f('icon')} style={sIconInput} />
              </div>
              <div className="adm-form-row">
            ...
            <button style={saving ? sSaveBtnDis : sSaveBtn} onClick={handleSave} disabled={saving}>
