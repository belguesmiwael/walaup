...
export default function AdminSidebar({ active, onTab, unreadCount = 0, pendingCount = 0, onLogout }) {
  ...
  const CSS = `
    ...
    .asb {
      ...
      overflow: visible;          /* visible so reopen tab sticks out */
      flex-shrink: 0;
    ...
    /* ── Reopen floating tab (only when closed) ── */
    .asb-reopen-tab {
      display: none;
      position: absolute;
      top: 50%;
    ...
    /* ── Nav ── */
    .asb-nav { flex: 1; padding: 6px 6px; overflow-y: auto; overflow-x: hidden; }
  ...
  return (
    <>
      ...
      <div className={`asb ${collapsed ? 'asb--closed' : 'asb--open'}`}>
        ...
        {/* ── Reopen floating tab (visible only when closed) ── */}
