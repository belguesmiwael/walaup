...
// ─── Style helpers (avoid   corruption)
const sStepLine    = (done, color) => ({ flex: 1, height: 2, borderRadius: 1, background: done ? color : 'rgba(255,255,255,0.06)' })
const sStepCircle  = (done, active, color) => ({
  width: 34, height: 34, minWidth: 34, borderRadius: '50%',
  display: 'flex', alignItems: 'center', justifyContent: 'center',
  transition: 'all .3s', zIndex: 1,
  background:  done ? color : active ? color + '22' : 'rgba(255,255,255,0.04)',
  border:      `2px solid ${done || active ? color : 'rgba(255,255,255,0.1)'}`,
  boxShadow:   active ? `0 0 14px ${color}60` : 'none',
})
const sStepLabel   = (done, active, color) => ({ fontSize: 10, color: active ? color : done ? 'var(--tx-2)' : 'var(--tx-3)', marginTop: 7, textAlign: 'center', lineHeight: 1.3, maxWidth: 70 })
const sStatusTagS  = (color) => ({ background: color + '18', color })
const sPulseDotS   = (color) => ({ width: 7, height: 7, borderRadius: '50%', background: color, display: 'inline-block' })
const sHistDotS    = (color) => ({ background: color + '22', border: `1.5px solid ${color}50` })
const sDemoBadgeS  = (bg, color) => ({ background: bg, color })
const sProjDotS    = (color) => ({ background: color })
const sProjStatusS = (color) => ({ background: color + '18', color })
...
function DemoPreviewModal({ url, onClose }) {
  ...
  const sWrap          = { background: 'rgba(13,17,32,0.98)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 18, overflow: 'hidden', maxWidth: '95vw', maxHeight: '95vh', display: 'flex', flexDirection: 'column' }
  const sMobilePad     = { padding: 24, display: 'flex', justifyContent: 'center', background: 'rgba(8,11,20,0.6)' }
  const sIframe        = { border: 'none', display: 'block' }
  const sBrowserBar    = { background: 'rgba(13,17,32,0.95)', borderBottom: '1px solid rgba(255,255,255,.06)', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 10 }
  const sDotRow        = { display: 'flex', gap: 5 }
  const sDotR          = { width: 9, height: 9, borderRadius: '50%', background: '#F87171', display: 'inline-block' }
  const sDotY          = { width: 9, height: 9, borderRadius: '50%', background: '#FBBF24', display: 'inline-block' }
  const sDotG          = { width: 9, height: 9, borderRadius: '50%', background: '#34D399', display: 'inline-block' }
  const sBrowserUrl    = { flex: 1, fontSize: 11, color: 'var(--tx-3)', fontFamily: "'JetBrains Mono',monospace", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }
  const sExtLink       = { color: 'var(--tx-3)' }
  const sDesktopIframe = { width: '90vw', maxWidth: 1100, height: '75vh', border: 'none', display: 'block', background: '#fff' }

  return (
    <div className="tp-prev-ov" onClick={onClose}>
      <div style={sWrap} onClick={e => e.stopPropagation()}>
        ...
        {mode === 'mobile' ? (
          <div style={sMobilePad}>
            <div className="tp-phone-shell">
              <div className="tp-phone-notch"/>
              <div className="tp-phone-screen" ref={screenRef}>
                <iframe ref={iframeRef} src={url} style={sIframe} title="Demo mobile"/>
              </div>
            </div>
          </div>
        ) : (
          <>
            <div style={sBrowserBar}>
              <div style={sDotRow}>
                <span style={sDotR}/><span style={sDotY}/><span style={sDotG}/>
              </div>
              <span style={sBrowserUrl}>{url}</span>
              <a href={url} target="_blank" rel="noreferrer"><ExternalLink size={13} style={sExtLink}/></a>
            </div>
            <iframe src={url} style={sDesktopIframe} title="Demo desktop"/>
          </>
        )}
      </div>
    </div>
...
function PaymentModal({ lead, amount, title, description, onClose, onConfirm, saving }) {
  ...
  const sEmoji         = { fontSize: 32 }
  const sMethodLabel   = { fontSize: 12, fontWeight: 700, color: 'var(--tx-2)', marginBottom: 9 }
  const sInstructions  = { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 14, marginBottom: 18 }
  const sInstrTitle    = { fontSize: 11, fontWeight: 700, color: 'var(--tx-2)', marginBottom: 10 }
  const sInstrItem     = { display: 'flex', gap: 8, marginBottom: 7, alignItems: 'flex-start' }
  const sInstrNum      = { width: 18, height: 18, borderRadius: '50%', background: 'rgba(99,102,241,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'var(--ac)', flexShrink: 0 }
  const sInstrText     = { fontSize: 12, color: 'var(--tx-2)', lineHeight: 1.5 }
  const sPayNote       = { fontSize: 11, color: 'var(--tx-3)', textAlign: 'center', marginTop: 14 }
  ...
  return (
    <div className="tp-modal-ov" onClick={onClose}>
      <div className="tp-modal" onClick={e => e.stopPropagation()}>
        ...
        <p style={sMethodLabel}>Choisissez votre méthode</p>
        ...
        {instructions[method] && (
          <div style={sInstructions}>
            <p style={sInstrTitle}>{selected?.emoji} Instructions — {selected?.label}</p>
            {instructions[method].map((step, i) => (
              <div key={i} style={sInstrItem}>
                <span style={sInstrNum}>{i+1}</span>
                <span style={sInstrText}>{step}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <p style={sPayNote}>Votre conseiller confirmera la réception du paiement sous 24h.</p>
      </div>
    </div>
  )
}
...
function HistoryTimeline({ lead, meta }) {
  ...
  if (histMap.validated)        events.push({ color: '#10B981', icon: CheckCircle2, title: `Démo validée (v${meta.validatedSlot || '?'})`, date: histMap.validated.date, desc: 'Vous avez approuvé cette version' })
  ...
  const sClockIcon = { marginRight: 3 }
  return (
        ...
        {events.map((ev, i) => {
          ...
          return (
            <div key={i} className="tp-hist-item">
              <div className="tp-hist-dot" style={sHistDotS(ev.color)}><Icon size={14} color={ev.color}/></div>
              <div className="tp-hist-content">
                <div className="tp-hist-title">{ev.title}</div>
                {ev.date && <div className="tp-hist-date"><Clock size={11} style={sClockIcon}/>{fmt(ev.date)}</div>}
...
function DemoViewer({ lead, setLead, meta }) {
  ...
  const persist = async (newMeta, extra = {}) => {
    ...
    // Realtime met à jour le lead automatiquement
  }
  ...
  const sValidatedTxt = { fontSize: 11, color: '#10B981', fontWeight: 600 }
          ...
          return (
            <div key={demo.slot} className={`tp-demo-card${isValidated?' tp-demo-card--validated':''}`}>
              <div className="tp-demo-head">
                ...
                <span className="tp-demo-badge" style={sDemoBadgeS(statusBg, statusColor)}>{statusLabel}</span>
              </div>
              <div className="tp-demo-actions">
                ...
                {isValidated && !isDelivered && <span style={sValidatedTxt}>Validée — paiement en attente</span>}
              </div>
            </div>
          )
        })}
...
function ProjectSelector({ leads, activeLead, onSelect }) {
  ...
  const STATUS_COLORS = { new:'#22D3EE', demo:'#6366F1', payment_requested:'#F59E0B', payment_confirmed:'#8B5CF6', delivered:'#10B981' }
  const STATUS_LABELS = { new:'Nouveau', demo:'Démo prête', payment_requested:'Paiement', payment_confirmed:'En dev.', delivered:'Livré' }
  return (
    <div className="tp-proj-selector">
      ...
      <div className="tp-proj-list">
        {leads.map(l => {
          const color = STATUS_COLORS[l.status] || '#22D3EE'
          const isActive = l.id === activeLead?.id
          const appName = l.note?.match(/App: ([^\n]+)/)?.[1]
          return (
            <div key={l.id} className={`tp-proj-item${isActive?' tp-proj-item--active':''}`} onClick={() => onSelect(l)}>
              <div className="tp-proj-dot" style={sProjDotS(color)}/>
              <div>
                <div className="tp-proj-name">{l.type || 'Application'}{appName ? ` — ${appName}` : ''}</div>
                <div className="tp-proj-pack">Pack {l.pack || '—'} · {new Date(l.created_at).toLocaleDateString('fr-FR')}</div>
              </div>
              <span className="tp-proj-status" style={sProjStatusS(color)}>{STATUS_LABELS[l.status] || l.status}</span>
            </div>
          )
        })}
      </div>
...
export default function TabProjet({ lead: initialLead, leads: allLeads, session, setLead }) {
  ...
  // Montant : BDD en priorité, sinon extraire depuis note, sinon config
  ...
  const sEmptyTx   = { fontSize: 15, fontWeight: 700, color: 'var(--tx)', marginBottom: 8 }
  const sEmptyP    = { fontSize: 13, color: 'var(--tx-3)', marginBottom: 20 }
  ...
  if (!lead) return (
    <>
      ...
      <div className="tp-empty">
        <Rocket size={40}/>
        <div style={sEmptyTx}>Aucun projet en cours</div>
        <p style={sEmptyP}>Décrivez votre besoin dans notre estimateur et recevez une démo en 48h.</p>
  ...
  const sPageWrap  = { padding: '20px 24px' }
  const sPageTitle = { fontSize: 18, fontWeight: 800, color: 'var(--tx)', fontFamily: "'Space Grotesk',sans-serif", marginBottom: 18 }
  const sActBtns   = { marginTop: 14, display: 'flex', gap: 10, flexWrap: 'wrap' }
  const sStatusMsg = { fontSize: 13, color: 'var(--tx-2)', lineHeight: 1.6, margin: 0 }
  ...
  return (
    <>
      ...
      <div style={sPageWrap}>
        <div style={sPageTitle}>Mon Projet</div>
            ...
            return (
              <div key={step.id} className="tp-step">
                <div className="tp-step-row">
                  {i > 0         && <div className="tp-step-line"   style={sStepLine(done, step.color)}/>}
                  <div className="tp-step-circle" style={sStepCircle(done, active, step.color)}>
                    {done ? <Check size={14} color="#fff"/> : <Icon size={15} color={active ? step.color : 'var(--tx-3)'} />}
                  </div>
                  {i < STEPS.length-1 && <div className="tp-step-line" style={sStepLine(done, step.color)}/>}
                </div>
                <span className="tp-step-label" style={sStepLabel(done, active, step.color)}>{step.label}</span>
              </div>
            )
          })}
        </div>
        ...
        {/* Status card */}
        <div className="tp-card">
          <span className="tp-status-tag" style={sStatusTagS(smeta.color)}>
            {status === 'payment_requested' && <span className="tp-pulse" style={sPulseDotS(smeta.color)}/>}
            {smeta.label}
          </span>
          <p style={sStatusMsg}>
