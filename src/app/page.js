...
const PAIN_POINTS = [
  { icon: '📄', title: 'Trop de papier', desc: 'Factures à la main, registres d\'erreurs. Chaque erreur vous coûte du temps et de l\'argent.' },
  { icon: '📦', title: 'Stock incontrôlable', desc: 'Vous ne savez jamais ce qu\'il reste. Ruptures, pertes et commandes ratées sans cesse.' },
  { icon: '💸', title: 'Pertes invisibles', desc: 'Erreurs de caisse, vols non détectés — l\'argent disparaît sans laisser de trace.' },
  { icon: '🔴', title: 'Clients impayés', desc: 'Dettes non suivies, trésorerie souffrante. Impossible de relancer sans système clair.' },
  { icon: '👥', title: 'Employés non suivis', desc: 'Présences, paiements, performance — tout dans votre tête ou sur un cahier.' },
  { icon: '📊', title: 'Aucune visibilité', desc: 'Vous ne savez pas si votre business est rentable. Les chiffres restent flous.' },
]

const SOLUTIONS = [
  { icon: '☕', name: 'App Café & Restaurant', features: ['Caisse anti-vol', 'Commandes par table', 'Gestion employés', 'Rapports quotidiens'], color: '#F59E0B' },
  { icon: '📦', name: 'App Stock & Inventaire', features: ['Entrées/sorties temps réel', 'Alertes de rupture auto', 'Historique mouvements', 'Multi-entrepôts'], color: '#10B981' },
  { icon: '💰', name: 'App Suivi Dettes', features: ['Clients débiteurs listés', 'Relances automatiques', 'Historique complet', 'Rapports mensuels'], color: '#6366F1' },
  { icon: '👶', name: 'App Crèche & École', features: ['Inscriptions en ligne', 'Présences & absences', 'Communication parents', 'Paiements intégrés'], color: '#EC4899' },
  { icon: '🚚', name: 'App Livraison', features: ['Commandes temps réel', 'Suivi livreurs GPS', 'Zones de livraison', 'Rapports détaillés'], color: '#22D3EE' },
  { icon: '🛒', name: 'E-commerce Tunisien', features: ['Boutique en ligne', 'Paiement Flouci/Konnect', 'Gestion commandes', 'Analytics intégrés'], color: '#8B5CF6' },
]
...
const WHY_US = [
  { icon: '📱', title: 'Disponible partout', desc: 'Mobile, tablette et PC. Accessible 24h/24, 7j/7 — même hors connexion via PWA.' },
  { icon: '🎯', title: '100% sur mesure', desc: 'Aucun template générique. Chaque écran est conçu uniquement pour votre business.' },
  { icon: '🇹🇳', title: 'Support local en arabe', desc: 'On est en Tunisie, on comprend votre réalité. Support en arabe et français.' },
  { icon: '✋', title: 'Simple à utiliser', desc: 'Si vous savez utiliser WhatsApp, vous savez déjà utiliser nos applications.' },
]
...
const PACKS = [
  {
    ...
    badge: null,
    color: '#6366F1',
    target: 'Petits business qui démarrent',
  ...
  },
  {
    ...
    badgeTx: '#000',
    color: '#F59E0B',
    target: 'Entrepreneurs ambitieux',
...
function HeroLivePreview() {
  const [phase, setPhase] = useState(0) // 0→building 1→interface 2→database 3→auth 4→done
    ...
    return (
      <div
        ...
        style={{
          ...
          borderRadius: 16,
          overflow: 'hidden',
          animation: 'wFadeUp 400ms ease forwards',
        }}
      >
        ...
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
            {[['Caisse', '1 420 DT', '#6366F1'], ['Stock', '847 DT', '#10B981'], ['Dettes', '320 DT', '#F59E0B'], ['Employés', '260 DT', '#8B5CF6']].map(([l, v, c]) => (
              <div key={l} style={{ background: 'var(--bg-elevated)', borderRadius: 10, padding: '10px 12px', borderLeft: `2px solid ${c}` }}>
          ...
          return (
            <div key={step} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              <div style={{
                ...
                background: done ? 'rgba(16,185,129,0.12)' : active ? 'var(--ac-glow)' : 'var(--bg-elevated)',
                border: `1.5px solid ${done ? 'rgba(16,185,129,0.4)' : active ? 'var(--ac)' : 'var(--border)'}`,
                transition: 'all 350ms ease',
                color: done ? '#10B981' : 'transparent',
              }}>
                {done ? '✓' : active ? (
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--ac)', animation: 'wPulse 0.8s ease-in-out infinite' }} />
                ) : null}
              </div>
              <span style={{ fontSize: 13, color: done || active ? 'var(--tx)' : 'var(--tx-3)', transition: 'color 350ms ease', flex: 1 }}>
                {step}
              </span>
              {active && (
                <div style={{ height: 3, width: 64, background: 'var(--bg-elevated)', borderRadius: 2, overflow: 'hidden' }}>
              ...
              {done && <span style={{ fontSize: 11, color: '#10B981' }}>✓</span>}
            </div>
          )
        })}
      </div>
...
function RevenueSimulator() {
  ...
  const partnerPct = 0.65
  const monthlyPerClient = 30
  const revenue = Math.round(clients * monthlyPerClient * partnerPct)

  return (
    <div style={{ marginTop: 20, padding: '18px', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 12 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--gold)', marginBottom: 10, letterSpacing: '0.04em' }}>💰 SIMULATEUR REVENUS PASSIFS</div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 11, color: 'var(--tx-3)', marginBottom: 2 }}>{clients} × {monthlyPerClient} DT × 65%</div>
        ...
        <div style={{ textAlign: 'right', fontSize: 11, color: 'var(--tx-3)', lineHeight: 1.5 }}>
          <div>sans effort</div>
          <div>scalable</div>
        </div>
      </div>
...
function AuthSection() {
  ...
  const handleGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: (typeof window !== 'undefined' ? window.location.origin : '') + '/client' },
    })
  ...
  const handleSendOtp = async () => {
    ...
    if (!cleaned || cleaned.length < 8) return setMsg({ type: 'error', text: 'Numéro invalide (8 chiffres minimum)' })
    setLoading(true); setMsg(null)
    const { error } = await supabase.auth.signInWithOtp({ phone: `+216${cleaned}` })
    if (error) setMsg({ type: 'error', text: error.message })
    else { setOtpSent(true); setMsg({ type: 'success', text: 'Code SMS envoyé sur +216 ' + cleaned }) }
    setLoading(false)
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 4, padding: 4, background: 'var(--bg-elevated)', borderRadius: 12, marginBottom: 24 }}>
        {[['google', '🔵 Google'], ['phone', '📱 Téléphone'], ['email', '✉️ Email']].map(([t, label]) => (
          <button
            key={t}
            onClick={() => { setMethod(t); setMsg(null) }}
            style={{
              flex: 1, padding: '9px 6px', borderRadius: 9, border: 'none', cursor: 'pointer',
              fontSize: 12, fontWeight: 600, fontFamily: 'var(--font-body)',
              background: method === t ? 'var(--bg-surface)' : 'transparent',
              color: method === t ? 'var(--tx)' : 'var(--tx-3)',
              boxShadow: method === t ? '0 1px 4px rgba(0,0,0,0.35)' : 'none',
      ...
      {msg && (
        <div style={{
          ...
          background: msg.type === 'error' ? 'rgba(248,113,113,0.1)' : 'rgba(16,185,129,0.1)',
          border: `1px solid ${msg.type === 'error' ? 'rgba(248,113,113,0.3)' : 'rgba(16,185,129,0.3)'}`,
      ...
      {method === 'google' && (
        <div style={{ textAlign: 'center' }}>
          <p style={{ color: 'var(--tx-2)', fontSize: 14, marginBottom: 24, lineHeight: 1.5 }}>
            Connexion en 1 clic avec votre compte Google.<br />
            <span style={{ color: 'var(--tx-3)', fontSize: 13 }}>Recommandé — rapide et sécurisé.</span>
          </p>
          <button
            ...
            style={{
              ...
              fontFamily: 'var(--font-body)', transition: 'all 200ms ease',
          ...
          {!otpSent ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ ...inputStyle, width: 'auto', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6, paddingRight: 12, fontSize: 13, color: 'var(--tx-2)' }}>
                  🇹🇳 +216
                </div>
                <input
                  type="tel" placeholder="XX XXX XXX" value={phone}
              ...
              <button
                ...
                style={{ width: '100%' }}
              >{loading ? 'Envoi...' : 'Envoyer le code SMS'}</button>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <input
                type="text" placeholder="Code à 6 chiffres" value={otp} maxLength={6}
                onChange={e => setOtp(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && handleVerifyOtp()}
                style={{ ...inputStyle, textAlign: 'center', letterSpacing: '0.4em', fontSize: 20, fontFamily: 'var(--font-mono)' }}
              />
              <button onClick={handleVerifyOtp} disabled={loading} className="btn btn-primary" style={{ width: '100%' }}>
                {loading ? 'Vérification...' : 'Vérifier le code'}
              </button>
              <button onClick={() => { setOtpSent(false); setMsg(null) }}
                style={{ width: '100%', padding: '9px', background: 'transparent', border: 'none', color: 'var(--tx-3)', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)' }}>
                ← Changer de numéro
              </button>
            </div>
          )}
        ...
        <div>
          <div style={{ display: 'flex', gap: 0, marginBottom: 20, borderBottom: '1px solid var(--border)' }}>
            {[['login', 'Se connecter'], ['register', 'Créer un compte']].map(([t, label]) => (
              <button
                key={t}
                onClick={() => { setAuthTab(t); setMsg(null) }}
                style={{
                  flex: 1, padding: '10px', border: 'none', background: 'transparent',
                  cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)',
                  color: authTab === t ? 'var(--ac)' : 'var(--tx-3)',
                  borderBottom: authTab === t ? '2px solid var(--ac)' : '2px solid transparent',
                  fontWeight: authTab === t ? 700 : 400, transition: 'all 200ms ease',
                }}>{label}</button>
            ))}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <button onClick={handleEmail} disabled={loading} className="btn btn-primary" style={{ width: '100%', marginTop: 4 }}>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    supabase.from('marketplace_apps').select('*').eq('active', true).limit(3)
      .then(({ data }) => {
        if (data?.length) {
          setMarketplaceApps(data.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0)))
        }
      })
  }, [])
  ...
  const authorName = currentTestimonial?.author || currentTestimonial?.author_name
  const authorRole = currentTestimonial?.role || currentTestimonial?.author_role
  const authorCompany = currentTestimonial?.company || currentTestimonial?.author_company
  const testimonialText = currentTestimonial?.text || currentTestimonial?.content
  const testimonialInitials = currentTestimonial?.initials || authorName?.slice(0, 2).toUpperCase()
  const testimonialColor = currentTestimonial?.color || 'var(--ac)'
  ...
  return (
    <>
      ...
      <style>{`
        ...
        @keyframes wGlowPulse { 0%,100% { box-shadow: 0 0 20px rgba(99,102,241,0.3); } 50% { box-shadow: 0 0 40px rgba(99,102,241,0.55); } }
        ...
        .pack-feature { display: flex; gap: 8px; align-items: flex-start; margin-bottom: 8px; font-size: 13px; }
        ...
        <div className="container">
          <div className="hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            ...
            <div style={{ animation: 'wFadeUp 600ms ease forwards' }}>
              {sectionTag('🚀 Agence Apps N°1 Tunisie')}
              <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem, 4.5vw, 3.6rem)', fontWeight: 700, lineHeight: 1.08, color: 'var(--tx)', marginBottom: 20 }}>
                ...
                <span className="text-gradient-ac">on crée votre application</span>
              </h1>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                <input
                  type="text"
                  placeholder="Ex : J'ai un café et je veux gérer ma caisse..."
                  value={heroInput}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 36 }}>
                {CHIPS.map(chip => (
                  <button key={chip} className="chip-btn" onClick={() => setHeroInput(chip)}>
                    {chip}
                  </button>
                ))}
              </div>
      ...
      <section style={{ padding: '36px 0', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
        <div className="container">
          <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, textAlign: 'center' }}>
            {[
              { val: '50+',   label: 'Clients satisfaits',     color: 'var(--ac)' },
              { val: '48h',   label: 'Délai démo',              color: 'var(--green)' },
              { val: '100%',  label: 'Made in Tunisia 🇹🇳',     color: 'var(--gold)' },
              { val: '0 DT',  label: 'Avant validation',        color: 'var(--ac-2)' },
            ].map(s => (
      ...
      {/* ══ 3. PAIN POINTS ══════════════════════════════════════════════ */}
      <section id="problems" className="sec-pad">
        <div className="container">
          <div className="sec-center">
            {sectionTag('😤 Problèmes courants')}
            <h2 style={h2}>Ces problèmes vous coûtent du temps<br />et de l'argent</h2>
            <p style={{ fontSize: 16, color: 'var(--tx-2)', maxWidth: 520, margin: '0 auto' }}>
              Si vous avez répondu oui à l'un de ces problèmes, on a la solution.
            </p>
          </div>
          <div className="pain-grid" data-stagger style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 }}>
            {PAIN_POINTS.map(p => (
              <div key={p.title} data-animate className="card card--interactive" style={{ padding: '22px', display: 'flex', gap: 14 }}>
                <span style={{ fontSize: 26, flexShrink: 0, lineHeight: 1.2 }}>{p.icon}</span>
                <div>
                  <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700, color: 'var(--tx)', marginBottom: 6 }}>{p.title}</h3>
                  <p style={{ fontSize: 13, color: 'var(--tx-2)', lineHeight: 1.55 }}>{p.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ══ 4. SOLUTIONS ════════════════════════════════════════════════ */}
      <section id="solutions" className="sec-pad" style={{ background: 'var(--bg-elevated)' }}>
        <div className="container">
          <div className="sec-center">
          ...
          <div className="sol-grid" data-stagger style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18, marginBottom: 40 }}>
            {SOLUTIONS.map(s => (
              <div key={s.name} data-animate className="card card--interactive card--tilt" style={{ padding: '24px' }}>
                <div style={{ fontSize: 30, marginBottom: 12 }}>{s.icon}</div>
                <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700, color: 'var(--tx)', marginBottom: 12 }}>{s.name}</h3>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {s.features.map(f => (
                    <li key={f} style={{ fontSize: 13, color: 'var(--tx-2)', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 7 }}>
                      <span style={{ color: s.color, fontSize: 8 }}>●</span>{f}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div style={{ textAlign: 'center' }}>
            <button onClick={scrollToContact} className="btn btn-primary btn-magnetic">Je veux mon app →</button>
          </div>
        </div>
      </section>
            ...
            {HOW_STEPS.map((step, i) => (
              <div key={step.n} style={{ display: 'flex', alignItems: 'flex-start', flex: 1 }}>
                <div data-animate style={{ flex: 1, textAlign: 'center', padding: '0 10px' }}>
                  <div style={{ width: 52, height: 52, borderRadius: '50%', border: '2px solid var(--ac)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', background: 'var(--ac-glow)', fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 700, color: 'var(--ac)' }}>
                    {step.n}
                  </div>
                ...
                {i < HOW_STEPS.length - 1 && (
                  <div className="step-line" style={{ flex: 0.4, height: 1.5, background: 'linear-gradient(90deg, var(--border-strong), transparent)', marginTop: 25 }} />
                )}
              </div>
            ))}
          </div>
      ...
      {/* ══ 6. WHY US ═══════════════════════════════════════════════════ */}
      <section className="sec-pad" style={{ background: 'var(--bg-elevated)' }}>
        <div className="container">
          <div className="sec-center">
            {sectionTag('🏆 Notre différence')}
          ...
          <div className="why-grid" data-stagger style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18, marginBottom: 44 }}>
            {WHY_US.map(w => (
              <div key={w.title} data-animate className="card" style={{ padding: '26px 18px', textAlign: 'center' }}>
                <div style={{ fontSize: 34, marginBottom: 14 }}>{w.icon}</div>
                <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 14, fontWeight: 700, color: 'var(--tx)', marginBottom: 8 }}>{w.title}</h3>
                <p style={{ fontSize: 13, color: 'var(--tx-2)', lineHeight: 1.55 }}>{w.desc}</p>
              </div>
            ))}
          </div>
          {/* Highlight box */}
          <div data-animate style={{ maxWidth: 680, margin: '0 auto', padding: '36px 40px', background: 'linear-gradient(135deg, rgba(99,102,241,0.07), rgba(139,92,246,0.07))', border: '1px solid var(--border-accent)', borderRadius: 20, textAlign: 'center', animation: 'wGlowPulse 4s ease-in-out infinite' }}>
            ...
            <p style={{ fontSize: 15, color: 'var(--tx-2)', lineHeight: 1.65, marginBottom: 26 }}>
              ...
              <strong style={{ color: 'var(--tx)' }}>Notre engagement unique.</strong>{' '}
              Personne d'autre ne propose ça en Tunisie.
            </p>
            <button onClick={scrollToContact} className="btn btn-primary btn-magnetic">Je veux ma démo gratuite →</button>
          </div>
        </div>
      </section>
      ...
      <section id="testimonials" className="sec-pad">
        <div className="container">
          ...
          <div style={{ maxWidth: 640, margin: '0 auto 28px' }}>
            <div key={testimonialIdx} className="card" style={{ padding: '36px', position: 'relative', overflow: 'hidden', animation: 'wFadeUp 300ms ease forwards' }}>
              <div style={{ position: 'absolute', top: 16, right: 24, fontSize: 72, opacity: 0.05, fontFamily: 'Georgia, serif', lineHeight: 1, userSelect: 'none' }}>"</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
                <div style={{ width: 46, height: 46, borderRadius: '50%', background: testimonialColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 700, color: '#fff', fontFamily: 'var(--font-display)', flexShrink: 0 }}>
                  {testimonialInitials}
                </div>
                <div>
                  <div style={{ fontWeight: 700, color: 'var(--tx)', fontSize: 15 }}>{authorName}</div>
                  <div style={{ fontSize: 13, color: 'var(--tx-2)' }}>{authorRole} · {authorCompany}</div>
                </div>
                <div style={{ marginLeft: 'auto', color: '#FBBF24', fontSize: 15, letterSpacing: 2 }}>★★★★★</div>
              </div>
              <p style={{ fontSize: 15, color: 'var(--tx)', lineHeight: 1.7, fontStyle: 'italic' }}>"{testimonialText}"</p>
            </div>
          </div>
          {/* Dots navigation */}
      ...
      <section id="marketplace" className="sec-pad" style={{ background: 'var(--bg-elevated)' }}>
        <div className="container">
          <div className="sec-center">
            {sectionTag('🏪 Marketplace')}
          ...
          <div className="mk-grid" data-stagger style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18, marginBottom: 40 }}>
            {(marketplaceApps.length > 0 ? marketplaceApps : SOLUTIONS.slice(0, 3)).map((app, i) => (
              <div key={app.id || app.name} data-animate className="card card--interactive card--tilt" style={{ padding: '24px' }}>
                <div style={{ fontSize: 30, marginBottom: 14 }}>{app.icon || SOLUTIONS[i]?.icon || '📱'}</div>
                ...
                {(app.tagline || app.description) && (
                  <p style={{ fontSize: 13, color: 'var(--tx-2)', marginBottom: 16, lineHeight: 1.5 }}>
                    {app.tagline || (app.description || '').slice(0, 90)}
                    {(app.description || '').length > 90 && '...'}
                  </p>
                )}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
                  {app.price_from ? (
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, fontWeight: 700, color: 'var(--gold)' }}>
                      dès {app.price_from} DT
                    </span>
                  ) : (
                    <span style={{ fontSize: 13, color: 'var(--tx-3)' }}>Prix sur demande</span>
                  )}
                  <Link href="/marketplace" style={{ fontSize: 13, color: 'var(--ac)', textDecoration: 'none', fontWeight: 600 }}>
                    Voir →
                  </Link>
                </div>
              </div>
          ...
          <div style={{ textAlign: 'center' }}>
            <Link href="/marketplace" className="btn btn-ghost">Voir toutes nos applications →</Link>
          </div>
        </div>
      </section>

      {/* ══ 9. PACKS ════════════════════════════════════════════════════ */}
      <section id="packs" className="sec-pad">
        <div className="container">
          <div className="sec-center">
            {sectionTag('💎 Nos packs')}
          ...
          <div className="packs-grid" data-stagger style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginBottom: 20 }}>
            {PACKS.map(pack => (
              <div key={pack.name} data-animate style={{ position: 'relative' }}>
                {pack.badge && (
                  <div style={{ position: 'absolute', top: -13, left: '50%', transform: 'translateX(-50%)', background: pack.badgeBg, color: pack.badgeTx, fontSize: 11, fontWeight: 700, padding: '4px 16px', borderRadius: 20, whiteSpace: 'nowrap', zIndex: 2 }}>
                    {pack.badge}
                  </div>
                )}
                <div className="card" style={{
                  padding: '32px 24px', height: '100%',
                  border: pack.name === 'Pro' ? '1px solid rgba(139,92,246,0.45)' : pack.name === 'Partenaire' ? '1px solid rgba(245,158,11,0.35)' : '1px solid var(--border)',
                  background: pack.name === 'Pro' ? 'linear-gradient(160deg, rgba(99,102,241,0.06), rgba(139,92,246,0.04))' : pack.name === 'Partenaire' ? 'linear-gradient(160deg, rgba(245,158,11,0.05), rgba(251,191,36,0.03))' : 'var(--glass-sm)',
                }}>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    {pack.features.map(f => (
                      <div key={f} className="pack-feature" style={{ color: 'var(--tx-2)' }}>
                        <span style={{ color: pack.color, flexShrink: 0, marginTop: 1 }}>✓</span>{f}
                      </div>
                    ))}
                    {pack.excluded.map(f => (
                      <div key={f} className="pack-feature" style={{ color: 'var(--tx-3)' }}>
                        <span style={{ flexShrink: 0, marginTop: 1 }}>✗</span>{f}
                      </div>
                    ))}
                  </div>
                  {pack.name === 'Partenaire' && <RevenueSimulator />}
                  <button onClick={scrollToContact} className={`btn ${pack.name === 'Pro' ? 'btn-primary' : 'btn-ghost'}`} style={{ width: '100%', marginTop: 20 }}>
                    Choisir {pack.name}
                  </button>
                </div>
              </div>
          ...
          <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--tx-3)' }}>
            Paiement uniquement après validation de votre démo. Aucun risque.
          </p>
        </div>
      </section>

      {/* ══ 10. ESTIMATEUR CTA ══════════════════════════════════════════ */}
      <section className="sec-pad-sm" style={{ background: 'var(--bg-elevated)', borderTop: '1px solid var(--border)' }}>
        <div className="container" style={{ textAlign: 'center', maxWidth: 640 }}>
          <div data-animate>
            {sectionTag('🧮 Estimateur')}
            <h2 style={{ ...h2, fontSize: 'clamp(1.5rem, 2.2vw, 2.2rem)' }}>Estimez le prix de votre app en 2 minutes</h2>
            <p style={{ fontSize: 15, color: 'var(--tx-2)', marginBottom: 28, lineHeight: 1.65 }}>
              Notre estimateur analyse vos besoins et vous donne une fourchette de prix précise + la formule recommandée. Gratuit, sans engagement.
            </p>
            <Link href="/estimateur" className="btn btn-primary btn-magnetic" style={{ display: 'inline-flex', gap: 8 }}>
              🧮 Calculer le prix de mon app →
            </Link>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="sec-pad">
        <div className="container">
          <div className="auth-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 72, alignItems: 'start', maxWidth: 940, margin: '0 auto' }}>
            {/* Left text */}
            <div data-animate>
              {sectionTag('🚀 Commencer maintenant')}
              <h2 style={{ ...h2, fontSize: 'clamp(1.8rem, 2.5vw, 2.6rem)' }}>Accédez à votre espace client</h2>
              <p style={{ fontSize: 15, color: 'var(--tx-2)', lineHeight: 1.65, marginBottom: 36 }}>
                Connectez-vous en 1 clic — puis créez ou suivez votre application sur mesure.
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                {[
                  { icon: '🆓', title: 'Démo 100% gratuite',    sub: '0 DT avant validation de votre application' },
                  { icon: '⚡', title: 'Réponse en 24 heures',  sub: 'Notre équipe vous contacte rapidement' },
                  { icon: '🤝', title: 'Sans engagement',       sub: 'Vous restez libre de décider à tout moment' },
                ].map(item => (
                  <div key={item.title} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                    <span style={{ fontSize: 22, flexShrink: 0, marginTop: 1 }}>{item.icon}</span>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--tx)', marginBottom: 2 }}>{item.title}</div>
                      <div style={{ fontSize: 13, color: 'var(--tx-2)' }}>{item.sub}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Auth card */}
            <div data-animate className="card" style={{ padding: '32px' }}>
              <AuthSection />
            </div>
          </div>
        </div>
      ...
      {/* ══ 12. FINAL CTA ═══════════════════════════════════════════════ */}
      <section className="sec-pad" style={{ background: 'var(--bg-elevated)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, maxWidth: 640 }}>
          <div data-animate>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem, 3.5vw, 3rem)', fontWeight: 700, color: 'var(--tx)', lineHeight: 1.15, marginBottom: 18 }}>
              Votre business mérite mieux<br />
              <span className="text-gradient-ac">qu'Excel et WhatsApp</span>
            </h2>
            <p style={{ fontSize: 16, color: 'var(--tx-2)', marginBottom: 36, lineHeight: 1.6 }}>
              Rejoignez les entrepreneurs tunisiens qui ont automatisé leur business avec Walaup.
            </p>
            <button onClick={scrollToContact} className="btn btn-primary btn-magnetic btn-lg">
